...
class _OrdersListState extends State<OrdersList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await OrdersProvider().schema();
    setState(() {});
  }
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, OrdersAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<Order> data) {
  ...
  Widget _buildCard(Order item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
...
Text('Items: ${item.items?.map((e) => e).join(", ")}'),
              ],
            ),
            trailing: Row(
