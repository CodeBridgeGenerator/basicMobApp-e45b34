...
class OrdersAdd extends StatefulWidget {
  const OrdersAdd({super.key, required this.resource});
  final List<Schema> resource;

  @override
  State<OrdersAdd> createState() => _OrdersAddState();
}
...
class _OrdersAddState extends State<OrdersAdd> {
  ...
  void saveForm() async {
    OrdersProvider provider = OrdersProvider();
    final data = Order.fromJson(formData);
    Response response = await provider.createOneAndSave(data);

    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully added Orders");
    } else {
      snackBar.FailedSnackBar(context, "Failed to add Orders");
    }
  }
