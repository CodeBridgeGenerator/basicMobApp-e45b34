...
class _ItemsListState extends State<ItemsList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await ItemsProvider().schema();
    setState(() {});
  }
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, ItemsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildCard(Item item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
