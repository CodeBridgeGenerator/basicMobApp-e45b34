...
class ItemsAdd extends StatefulWidget {
  const ItemsAdd({super.key, required this.resource});
...
class _ItemsAddState extends State<ItemsAdd> {
  ...
  void saveForm() async {
    ...
    final data = Item.fromJson(formData);
    Response response = await provider.createOneAndSave(data);

    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully added Items");
    } else {
      snackBar.FailedSnackBar(context, "Failed to add Items");
    }
  }
