...
class ProductsAdd extends StatefulWidget {
  const ProductsAdd({super.key, required this.resource});
...
class _ProductsAddState extends State<ProductsAdd> {
  ...
  void saveForm() async {
    ...
    final data = Product.fromJson(formData);
    Response response = await provider.createOneAndSave(data);

    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully added Products");
    } else {
      snackBar.FailedSnackBar(context, "Failed to add Products");
    }
  }
