...
class _ProductsListState extends State<ProductsList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await ProductsProvider().schema();
    setState(() {});
  }
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, ProductsAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<Product> data) {
  ...
  Widget _buildCard(Product item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
