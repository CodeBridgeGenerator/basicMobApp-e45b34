...
import 'CustomersAdd.dart';
import 'CustomersEdit.dart';
...
class _CustomersListState extends State<CustomersList> {
  ...
  Future<void> _fetchSchema() async {
    schemaResponse = await CustomersProvider().schema();
    setState(() {});
  }
  ...
  Future<void> _delete(String id, bool confirm) async {
    if (confirm) {
      ...
      if (response.isSuccess) {
        ...
        setState(() {
          provider.loadCustomersFromHive();
        });
      } else {
  ...
  AppBar _buildAppBar() {
    return AppBar(
      title: const Text('Customers Details'),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.of(context).push(Utils().createRoute(
                  context, CustomersAdd(resource: schemaResponse?.data)));
            },
  ...
  Widget _buildList(List<Customer> data) {
  ...
  Widget _buildCard(Customer item, int index) {
    return Card(
      margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Column(
        children: [
