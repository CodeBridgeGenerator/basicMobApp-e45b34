...
class CustomersAdd extends StatefulWidget {
  const CustomersAdd({super.key, required this.resource});
...
class _CustomersAddState extends State<CustomersAdd> {
  ...
  void saveForm() async {
    CustomersProvider provider = CustomersProvider();
    final data = Customer.fromJson(formData);
    Response response = await provider.createOneAndSave(data);

    SnackBars snackBar = SnackBars();
    if (response.isSuccess) {
      snackBar.SuccessSnackBar(context, "Successfully added Customers");
    } else {
      snackBar.FailedSnackBar(context, "Failed to add Customers");
    }
  }
