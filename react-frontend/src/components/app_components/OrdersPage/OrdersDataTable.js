...
const multiselectTemplate2 = (rowData, { rowIndex }) => (<ul className="list-none" >{rowData.products.map((i,x) => (<li key={x}> orderNumber: {i.orderNumber},customer: {i.customer}</li>))}</ul>)
...
<Column field="items" header="Items" body={multiselectTemplate2} filter={selectedFilterFields.includes("items")} hidden={selectedHideFields?.includes("items")}  style={{ minWidth: "8rem" }} />
